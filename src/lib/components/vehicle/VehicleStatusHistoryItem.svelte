<script lang="ts">
	...
	interface Props {
		item: Vehicle.VehicleStatusChange;
	}

	let { item }: Props = $props();

	let config = $derived(vehicleStatusMap[item.status as Vehicle.Status]);
...
<div class="timeline-block my-3">
	...
	<button class="btn btn-clear timeline-content mw-100" onclick={openDetails} type="button">
