<script lang="ts">
	...
	const upload = async () => {
		...
		if (isUnidentified) {
			...
			result = {
				name: vehicleDocumentNames[type],
				type,
...
<ClosableModal bind:isOpen={showUploader} headerText="Wgraj pliki" onClick={upload} buttonCaption={uploadable && 'Zapisz'} size="lg">
	...
	{#if result}
		<section class="mt-3 pt-3 border-top text-dark text-center fs-5">
			Zidentyfikowano plik jako <b>{vehicleDocumentNames[result.type]}</b>
				...
				{#each vehicleDocumentCategories as cat}
					<div class="col-12 col-md-6 mb-3">
						...
						{#each cat.fields as type}
							<CustomFormRadio name="unidentifiedDcumentType" value={type} caption={vehicleDocumentNames[type]} onChange={onSelectTypeChange} />
						{/each}
					</div>
				{/each}
			</div>
