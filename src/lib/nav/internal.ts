...
export const fixSubRoute = (url: string) => {
    const subRouteUrl = getStore(subRoute);
    if (!subRouteUrl.length) return url;
    if (url.startsWith(subRouteUrl)) return url;
    return subRouteUrl + url;
}

const get = async (url: string, data: Record<string,any> = {}, config: Record<string,any> = {}, apiRoute: boolean = false) => {
    if (!browser) return;
    const subRouteUrl = apiRoute ? url :fixSubRoute (url.replace(/([^:])\/\//, '$1/'));
    config.params = data;
    const apiReq = setApiRequest('GET', url, data)
    const response = await axios.get(subRouteUrl, config).catch(catchApiReqError(apiReq));
    if (!response) return;
...
const post = async (url: string, data: Record<string,any> = {}, config: Record<string,any> = {}, method: 'post' | 'patch' | 'delete' = 'post', apiRoute: boolean = false) => {
    const subRouteUrl = apiRoute ? url : fixSubRoute(url);
    if (!browser) return;
    config.headers = headers;
    const apiReq = setApiRequest(method.toUpperCase(), url, data)
    const response = await axios[method](subRouteUrl, data, config).catch(catchApiReqError(apiReq));
    if (response) {
        setApiResponse(response, apiReq);
        return response.data;
    }
...
const download = async (url: string, data: Record<string,any>, config: Record<string,any> = {}) => {
    if (!browser) return;
    const subRouteUrl = apiRoute ? url :getStore(subRoute) + url;
    config.headers = headers;
    config.headers['x-api-download'] = 1;
    config.responseType = 'blob';
    const response = await axios.post(subRouteUrl, data, config).catch(err => err.response);
    return response;
}
...
const del = async (url: string, data: Record<string,any> = {}, config: Record<string,any> = {}, apiRoute: boolean = false) => {
    ...
    const response = await axios.delete(subRouteUrl, config).catch(catchApiReqError(apiReq));
    if (response) {
        setApiResponse(response, apiReq);
        return response.data;
    }
