...
export function requireAuth(locals: App.Locals): App.User | App.UserBase | null {
	if (!CHECK_AUTH) return locals.user || locals.driver;
	if (locals.userType === 'admin') return restrictAdmin(locals);
	if (locals.userType === 'driver') return restrictDriver(locals);
